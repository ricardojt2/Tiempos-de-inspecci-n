...
ruta_aux <- 'C:/Users/1998R/OneDrive/Escritorio/Datos'#'AQUI VA RUTA'

source(paste0(ruta_aux,'/binario a censura.R'))
source(paste0(ruta_aux,'/parte1.R'))
source(paste0(ruta_aux,'/parte2.R'))
#_seleccionar_cuantil
p_auxp <- 0.5 #selecciona cuantil
source(paste0(ruta_aux,'/parte3.R'))
...
pre_ecm <- function(tiempo){
  ...
  for(j in 1:m){  #cambiar tiempo de ce2 y ce3 (rel. con la distribucion de los tiempos de inspeccion)
    ...
    t_data <- data.frame(start = TOSTADA[,1],stop=TOSTADA[,2],event=rep(3,num),TEM=tem,id = 1:num)
     ...
     Out <- try( survreg(Surv(start,stop,event,type='interval',origin = -0.0001) ~ TEM ,data=t_data ,dist="weibull", control = list(maxiter=1000)),
                silent = TRUE)
    # A <-survreg(Surv(start,stop,event,type='interval',origin = -0.0001) ~ age ,data=t_data ,dist="weibull", control = list(maxiter=1000))
    A <- summary(Out)
    print(A$table)
  ...
  if(length(por_si[1]) > 0){
    ...
    estimacion1 <- (sqrt(estimacion1)/real1)*1
    estimacion2 <- (sqrt(estimacion2)/real2)*1
    estimacion3 <- (sqrt(estimacion3)/real3)*1
...
#cuantil 0.5 
#USAR Y TAMBIEN EN OP LINEA 144 SI YA SE GUARDO LOS RESULTADOS 
#FF3 <- data.frame(read.csv('historial5ecmr1.csv')) 
#FF3 <- FF3[ , !(names(FF3) %in% c('X','Round') ) ]

n_opt <- 5 #numero de optimizaciones
...
op <-BayesianOptimization(f_min2,
                          bounds = donde_busca, #init_grid_dt = FF3,
...
#guardar cosas
#mejor_parametros <- write.csv(op$Best_Par,'mejores_parametros5ecmr3.csv')
#mejor_parametros
#mejor_valor <- write.csv(op$Best_Value,'mejor_valork5ecmr3.csv')
#mejor_valor
historial <- write.csv(datos_a_guardar,'historial5ecmr1ka5.csv')
historial
